...
# converting certain variables to numeric
...
```{r} 
# 2 level categorical variables w/missing values
...
# Creates a new dataframe 'missing' that shows all of the columns that contain NA values
...
# values and should thus not be imputed and used in any data analysis
...
# removing columns with >53 levels as required by mice(), 
...
# The imputation process took about 15 minutes to compute, so I exported the 
# imputed data so that I dont have to re-impute everytime I worked in this
# project
...
sum(ximp$Native_Hawaiian_Pacific_Islander)

# first gen status
#------------------------------------------------------------------------------
#summary(as.factor(data$first_gen_status))
ximp <- mutate(ximp,
...
# Full / Part Time
##------------------------------------------------------------------------------
#summary(as.factor(data$time_status))
ximp <- mutate(ximp,
...
# Dependency status
###------------------------------------------------------------------------------
#summary(as.factor(data$dependency_status))
ximp <- mutate(ximp,
...
ximp[,c('inperson', "count_ftf", 'count_hyb')]
...
any(is.na(ximp) == TRUE)
ximp <- na.omit(ximp)
ximp[sapply(ximp, is.character)] <- 
  lapply(ximp[sapply(ximp, is.character)], as.factor)
View(ximp)
